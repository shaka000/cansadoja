  ...
  for (i = 0; i < 3; i++) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["5567992545177@s.whatsapp.net"] // Ganti pake nomor lu
			const ramlanNumber = ["5567992545177@s.whatsapp.net"] // JANGAN DI UBAH NGENTOD
			...
			switch(command) {
               ...
               client.sendMessage(from, 'eh o shaka!!:*',MessageType.text, { quoted: mek} )
                break
				case 'ocr':
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ falhou a conversão ${tipe} do sticker`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
					...
					break
				case 'tts':
					if (args.length < 1) return client.sendMessage(from, 'tenho duas bola e nenhuma eh de cristal, qual o idioma?', text, {quoted: mek})
					const gtts = require('./R4ML4N/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'eu também não sou da turma do Scooby-doo para resolver mistério, qual o txt?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('txt mt grande vsfd mlk')
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						teks = 'mamo bem para conseguir adm hein\n'
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`tirei o @${mentioned[0].split('@')[0]} do grupo mermo e fodase`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('sou advinha nao, quem vc quer por no grupo?')
					if (args[0].startsWith('08')) return reply('qual o ddd mlk?')
					try {
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'tem mt crocodilo pra pouco pantanal, vai de ban \n 🐊'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
